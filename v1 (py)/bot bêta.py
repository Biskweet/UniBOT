...
from variables import (SU_hex, discord_channels, EIGHTBALL_ANSWERS, icons, headers, alphabet)
...
def hasquoi(msg):
    return ''.join([char for char in msg if char in alphabet]) == "quoi"
# =========================================
...
async def on_message(message):
    if message.author != client.user:
        ...
        if message.author.id == 215427306053828608 and "mod" in message.content:
...
async def on_member_join(member):
    ...
    if member.guild.name == "xyz":
        
        embed = make_embed(title       = ":wave: Bienvenue dans le serveur Discord Sorbonne Université !",
                           description = "")

        await member.send(f"({round(client.latency * 100)} millisecondes)")


@client.event
async def on_member_remove(member):
    channel = client.get_channel(discord_channels["leave"])
    await channel.send(f"<@{member.id}> a quitté le serveur.")
# =========================================
...
"""
        ...
        description = "Effectue une requête et renvoie l'introduction d'un article Wikipédia.\n" \
                      "Prend en paramètre (optionnel) le titre de l'article, cherche la correspondance la plus proche et renvoie " \
                      "l'introduction de celle-ci. Si aucun titre n'est fourni, renvoie un article choisi au hasard.\n\n" \
                      "Exemple d'utilisation :\n`unibot wiki` (article au hasard)\n`unibot wiki victor hugo`"
...
async def wiki(ctx, *, title: str = ''):
    ...
    if not title:
        await ctx.send(embed=make_embed(description="Article aléatoire"))
        req = r.get("https://fr.wikipedia.org/w/api.php?action=query&generator=random&prop=extracts"
        ...
        link = "https://fr.wikipedia.org/wiki/" + content["title"].replace(' ', '_')
        ...
        search, articles, _, link = r.get("https://fr.wikipedia.org/w/api.php?action=opensearch&limit=1&search=" + title).json()
        if articles:
            article = r.get("https://fr.wikipedia.org/w/api.php?format=json&action=query&"
...
async def update_member_count():
    await client.wait_until_ready()
    member_count = len(client.get_guild(DSU_ID).members)
...
client.run("ODc3NDk0Mjg4OTExNTY0ODUy.YRzcPQ.CLiGzQLecYUCHu3MBfG-Uw33dt8")


"""
r.get("https://api.twitter.com/2/users/940938863790428160/tweets?max_results=5&tweet.fields=referenced_tweets&&expansions=attachments.media_keys&media.fields=preview_image_url,type,url", headers=headers)
"""
